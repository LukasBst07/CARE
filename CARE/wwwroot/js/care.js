...
(function () {
    ...
    // ── Category boards update (Ranking page) ──────────────
    window.updateCategoryBoards = function (classrooms) {
        const cats = [
            { prop: 'scoreEco2', sort: 'scoreEco2' },
            { prop: 'scoreDb', sort: 'scoreDb' },
            { prop: 'scoreLux', sort: 'scoreLux' },
            { prop: 'scoreTemp', sort: 'scoreTemp' },
            { prop: 'scoreHumidity', sort: 'scoreHumidity' },
        ];

        const boards = document.querySelectorAll('.cat-board');
        boards.forEach((board, bi) => {
            const cat = cats[bi];
            if (!cat) return;
            const sorted = [...classrooms].sort((a, b) => (b[cat.sort] || 0) - (a[cat.sort] || 0));
            const rows = board.querySelector('.cat-rows');
            if (!rows) return;

            rows.innerHTML = '';
            sorted.slice(0, 4).forEach((cls, i) => {
                const m = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `#${i + 1}`;
                const dotColor = cls.online ? '#00ff88' : '#ffffff22';
                const score = cls[cat.sort] || 0;
                const a = document.createElement('a');
                a.href = `/Classroom/Detail/${cls.name}`;
                a.className = 'cat-row';
                a.innerHTML = `
          <span class="cat-medal">${m}</span>
          ...
          <span class="cat-dot" style="background:${dotColor}"></span>`;
            ...
            if (sorted.length === 0) {
                rows.innerHTML = '<div class="font-mono text-dim" style="padding:1rem;text-align:center">NO DATA</div>';
            }
        });
    };
    ...
    function pollLiveData() {
        fetch('/api/live')
            ...
            .then(data => {
                ...
                // Fire all update functions
                if (typeof updateBattleGrid === 'function') updateBattleGrid(data);
                if (typeof updateWinnerBanner === 'function') updateWinnerBanner(data);
                if (typeof updateLeaderboard === 'function') updateLeaderboard(data);
                if (typeof updateCategoryBoards === 'function') updateCategoryBoards(data);

                const lastUpEl = document.getElementById('lastUpdate');
                if (lastUpEl) lastUpEl.textContent = new Date().toLocaleTimeString();
            })
            .catch(err => console.warn('[AJAX Poll] Error:', err));
    }

    // Start polling after page load
    setInterval(pollLiveData, 5000);
    // Initial fetch after 1s (let page render first)
